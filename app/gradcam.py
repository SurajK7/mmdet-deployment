from fastai.vision import load_learner, open_image, sys, Path, Image, callbacks, torch
...
class GradCam():
    ...
    def from_one_img(cls,learn,x_img,label1=None):
        pred_class, _, probs = learn.predict(x_img)
    ...
    def plot(self,plot_hm=True,plot_gbp=False):
        if not plot_hm and not plot_gbp:
            plot_hm=True
        rows = 4 if hasattr(self, 'label2') else 2
        ...
        if plot_hm:
            show_heatmap(self.hmap1,self.xb_img,size,axes)
            axes.set_title(f'Heatmap for {self.label1}');row+=1
        if plot_gbp:
            axes[row].imshow(self.xb_grad1)
            axes[row].set_axis_off()
            axes[row].set_title('Isolating high interest regions')
        plt.tight_layout()
        fig.savefig(f'/tmp/gradcam_image_{self.label1}.png')
